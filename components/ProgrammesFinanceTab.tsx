...
import { ProgrammeOpportunity, AdvisoryOrder } from '@/types';
import { getProgrammeType } from '@/lib/programmes';

interface Props {
  opportunities: ProgrammeOpportunity[];
...
export default function ProgrammesFinanceTab({ opportunities, orders, lastUpdated }: Props) {
  const [progType, setProgType]       = useState<'All' | 'fellowship' | 'exchange' | 'ltd' | 'other'>('All');
  ...
  const filtered = useMemo(() => {
    return rows
      .filter(r => progType === 'All' || r.type === progType)
  ...
  const chartData = useMemo(() =>
    FY_MONTHS.map(({ year, month, label }) => {
      ...
      for (const { opp, order } of filtered) {
        if (closesInMonth(opp, year, month)) {
          won += opp.Amount ?? 0;
        }
    ...
    <div className="space-y-5">
      ...
      <div className="fi-card">
        <div className="flex items-start justify-between gap-4 mb-1">
          <div>
            ...
            <p className="text-xs text-[#8a7a6a] font-[Geist] mt-0.5">
              {showCumulative ? 'Cumulative' : 'Monthly'} · {filtered.length} programme{filtered.length !== 1 ? 's' : ''} · data as of {new Date(lastUpdated).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
            </p>
          </div>
          <button
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={displayData} margin={{ top: 12, right: 8, left: 0, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="won"      name="Won (confirmed at close)"     stroke="#195e47" strokeWidth={2} strokeDasharray="6 3" dot={false} activeDot={{ r: 4, fill: '#195e47', strokeWidth: 0 }} />
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-2 mt-3 pt-3 border-t border-[#e8ddd0]">
          <span className="flex items-center gap-2 text-xs font-[Geist]">
            ...
            <span className="text-[#8a7a6a]">— confirmed income recognised at close date (lump sum)</span>
          </span>
          <span className="flex items-center gap-2 text-xs font-[Geist]">
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-[#8a7a6a] font-[Geist] uppercase tracking-widest">Type</span>
          {(['All', 'fellowship', 'exchange', 'ltd', 'other'] as const).map(t =>
            pill(progType === t, () => setProgType(t),
              t === 'All' ? 'All' : t === 'fellowship' ? 'Fellowship' : t === 'exchange' ? 'Exchange' : t === 'ltd' ? 'LTD' : 'Other'),
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
      ...
      <div className="flex items-center gap-2">
        ...
        {(['close', 'org', 'amount'] as SortKey[]).map(key =>
          pill(sortKey === key, () => setSortKey(key), key === 'close' ? 'Close date' : key === 'org' ? 'Organisation' : 'Amount'),
        )}
      </div>
